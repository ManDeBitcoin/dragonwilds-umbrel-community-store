...
async function api(req, res, url) {
  ...
  const worldActionsMatch = url.pathname.match(/^\/api\/worlds\/([^/]+)\/actions$/);
  if (req.method === "GET" && worldActionsMatch) {
    const worldName = decodeURIComponent(worldActionsMatch[1]);
    const actions = runtime.playerDatabase ? runtime.playerDatabase.getActionLog(worldName) : [];
    return json(res, 200, { worldName, actions });
  }
